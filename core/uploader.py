...
def upload_file(provider, local_path, remote_relative_path, retries=3, delay=5):
    return provider.upload_file(local_path, remote_relative_path, retries=retries, delay=delay)


def download_file(provider, remote_relative_path, local_path, retries=3, delay=5):
    return provider.download_file(remote_relative_path, local_path, retries=retries, delay=delay)
